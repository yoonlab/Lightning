...
{
	int iTotalCellNum = iGridSize * iGridSize ;
		...
		for ( int i = 0; i < iTotalCellNum; ++i )
		{
			if ( vCells[ i ] )
			{
				...
				for ( int dir = E_ND_UP; dir < MAX_NEIGHBOR_DIRECTION; ++dir )
				{
					...
					iNeighborIndex = iNeighborY * iGridSize + iNeighborX ;
...
int		CGMSolver::Solve_Old( std::vector< Cell* >& vCells, int iGridSize, int iMaxIteration )
{
	int iTotalCellNum = iGridSize * iGridSize ;
	if ( vCells.size() < iTotalCellNum )
	{
	...
	new_r.assign( iTotalCellNum, 0.0f ) ;
	...
	int k = 0 ;
	for ( ; k < iMaxIteration; ++k )
	{
		// calculate alpha
		alpha = CalcAlpha( vCells, p, iGridSize ) ;
		...
		for ( int i = 0; i < iTotalCellNum; ++i )
		{
			if ( vCells[ i ] )
			{
				vCells[ i ]->m_fPotential += alpha * p[ i ] ;
		...
		for ( int i = 0; i < iTotalCellNum; ++i )
		{
			if ( vCells[ i ] )
			{
				...
				new_r[ i ] = vCells[ i ]->m_fR - alpha * q[ i ] ;
				if ( fabs( new_r[ i ] ) > fMaxR )
				{
					fMaxR = fabs( new_r[ i ] ) ;
				}
			}
		...
		// calculate beta
		beta = CalcBeta( vCells, new_r, iGridSize ) ;
...
{
	if ( vCells.size() < iGridSize * iGridSize )
	{
		std::cerr << std::endl ;
		std::cerr << "cell size error !! " << std::endl ;
	...
	for ( int i = 0; i < iGridSize; ++i )
	{
		for ( int j = 0; j < iGridSize; ++j )
		{
			...
			if ( vCells[ iIndex ] )
			{
				...
				for ( int dir = E_ND_UP; dir < MAX_NEIGHBOR_DIRECTION; ++dir )
				{
					iNeighborX = j + Cell::DIR_X_DIFF[ dir ] ;
					iNeighborY = i + Cell::DIR_Y_DIFF[ dir ] ;
					iNeighborIndex = iNeighborY * iGridSize + iNeighborX ;
					...
					if ( iNeighborX < 0 || iNeighborY < 0 )
					{
						// left & top boundary
						fBoundarySum += 0.0f * fInvDx ;
					}
					else if ( iNeighborX >= iGridSize || iNeighborY >= iGridSize )
					{
